...
A : 가뭄피해가 많은 지역에서 겪은 가뭄피해 원인은 제한급수였다. 그리고 지역적으로 광범위하게 나타나는 가뭄피해 유형은 운반급수이다. 

Q3. 가뭄 피해 기간은 시군구에 따라 어떤 차이를 보이는가?
![alt text](./image/image17.png)

A : 완도군의 피해 기간이 가장 길다. 반대로 강원도에서 가장 많은 가뭄 피해 인구를 기록했던 속초시는 가뭄 피해를 겪었던 기간이 그리 길지 않다. 오히려 가평군이 완도 다음으로 길다.

## 3-7 워드 클라우드
##### *단어 분석 중심의 시각화*

워드 클라우드란 말 그대로 다양한 단어가 구름처럼 떠있는 차트. 그렇다고 단어가 그저 아무렇게나 떠있는 건 아님. 중요도에 따라 크기가 다르게 표시된다.
...
> ## TIP 워드 클라우드르 사용할 때 알아 두면 좋은 내용

### tip1. 단어가 많다면 빈도수가 낮은 단어들은 제외하자.

### tip2. 별 의미 없는 단어는 제거하자.

> # 직접 해보기 뉴스에서 가장 많이 언급되는 단어는 무엇인가?

![alt text](./image/image18.png)

A : 2012년에 가장 많이 언급된 토픽은 후보, 민주, 대선, 대통령과 같은 대선 관련 단어가 많이 등장한다.

> 고민해보기 : 다양한 워드 클라우드로 질문에 답해 보기
Q 시간의 흐름에 따라 토픽의 빈도수에 차이가 있는가?

![alt text](./image/image19.png)

![alt text](./image/image20.png)

1월에 봉투와 민주라는 단어 가장 많이 언급됨

2월에는 민주와 공천 또한 그리스라는 토픽 단어가 보이는것으로 미루어 보았을때, 대선을 앞두고 공천이 시작되었고, 그리스 경제 문제가 시작되었다.